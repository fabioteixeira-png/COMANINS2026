...
const oldEffectStart = content.indexOf('// START TEMP MIGRATION');
const oldEffectEnd = content.indexOf('// END TEMP MIGRATION') + '// END TEMP MIGRATION'.length;
if (oldEffectStart !== -1 && oldEffectEnd !== -1) {
  content = content.slice(0, oldEffectStart) + content.slice(oldEffectEnd);
}
...
const buttonCode = `
        ...
        onClick={async () => {
          try {
            ...
            for (const d of instSnap.docs) {
              const data = d.data();
              if (targetIds.includes(data.certificateNumber) || targetIds.includes(data.id) || targetIds.includes(data.tag)) {
      ...
      >
        ⚠️ CLIQUE AQUI PARA EXECUTAR CORREÇÃO DE INSTRUMENTOS (NSM -> COMANINS) ⚠️
      </button>
`;
