...
const regex = /app\.post\("\/api\/auth\/legacy-login", async \(req, res\) => \{[\s\S]*?res\.status\(500\)\.json\(\{ error: "Internal server error" \}\);\s*\}\);/;

code = code.replace(regex, `app.post("/api/auth/legacy-login", async (req, res) => {
  try {
